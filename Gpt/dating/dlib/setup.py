...
from setuptools import log  # distutils 대신 setuptools 사용
...
def read_version_from_cmakelists(cmake_file):
    ...
    """
    major = re.findall(r"set\(CPACK_PACKAGE_VERSION_MAJOR.*\"(.*)\"", open(cmake_file).read())[0]
    minor = re.findall(r"set\(CPACK_PACKAGE_VERSION_MINOR.*\"(.*)\"", open(cmake_file).read())[0]
    patch = re.findall(r"set\(CPACK_PACKAGE_VERSION_PATCH.*\"(.*)\"", open(cmake_file).read())[0]
